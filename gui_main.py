...
UI_CONFIG = {
    ...
    'spinbox_rounds_min': 10,        # 轮次 Spinbox 最小值
...
class BossFilterGUI:
    ...
    def create_run_page(self):
        ...
        ttk.Spinbox(row1, from_=UI_CONFIG['spinbox_rounds_min'], to=UI_CONFIG['spinbox_rounds_max'], textvariable=self.rounds_var, width=15, font=self.font_button).pack(side="left", padx=int(15 * self.dpi_scale * self.zoom_factor))
    ...
    def use_selected_model(self):
        ...
        if model_config:
            # 检查 API Key 是否存在
            saved_api_key = model_config.get("api_key", "")
            if not saved_api_key:
                # 已保存的配置中没有 API Key，使用当前输入的
                saved_api_key = self.api_key_var.get().strip()
                if not saved_api_key:
                    messagebox.showwarning("警告",
                        f"模型 '{model_name}' 的配置中没有保存 API Key\n\n"
                        f"请在上方输入 API Key 后重新保存该模型")
                    return
            ...
            try:
                ...
                try:
                    ...
                    if 'zhipin.com/web/chat/recommend' in current_url.lower():
                        ...
                        self.browser_status_help.config(text=f"已连接到 BOSS 直聘推荐页面")
                        self.start_btn.config(state="normal")  # 启用开始按钮
                        ...
                        self.start_btn.config(state="disabled")  # 需导航时禁用开始按钮
                    ...
                    self.start_btn.config(state="disabled")  # 未连接时禁用开始按钮
