...
- **手动打招呼不再依赖浏览器位置**：筛选时会自动保存候选人的联系凭证；之后在界面手动打招呼时直接发送，不用再切回推荐牛人页面；发送失败时自动回退到列表按钮方式，确保打招呼不中断
- **重启后仍可继续发送**：联系凭证随候选人数据一起保存，关闭程序再打开仍可直接发送

**体验优化**

- **上下文采集更克制**：只对评分较高、数量有限的高价值候选人采集凭证，降低被平台风控的概率
- **人工核实候选人也立即可用**：标记"需人工确认"的候选人会提前采集凭证；核实通过后点"打招呼"立即发送，不再依赖浏览器位置
- **扫描更全面**：单次扫描能拿到的候选人信息更完整，深页候选人覆盖率提升
- **日志更易读**：扫描完成时能看到不同来源的贡献人数，便于判断扫描效果
- **Excel 导出更稳**：导出前自动清洗候选人信息中的异常字符，避免个别脏数据让整张 Excel 崩溃
