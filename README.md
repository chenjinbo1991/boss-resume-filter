...
- **数据统计看板**：侧边栏新增「数据统计」页，按岗位维度聚合筛选和打招呼数据，支持时间范围过滤
- **选择器健壮性加固**：所有页面 CSS/XPath 选择器集中到 `selectors.json`，BOSS 前端 DOM 变化时只改配置文件；浏览器连接后自动检查选择器健康状态
- **候选人明细弹窗增强**：点击统计指标数字弹出明细窗口，新增「推荐指数」列和右键菜单（查看详情/移除/导出），弹窗相对主窗口居中
- **清空候选人保留打招呼**：清空操作新增"保留已打招呼的候选人"复选框（默认勾选），清空时自动保留已沟通过的记录；清空后同步更新 Excel

### v2.6 新增功能
- **筛选逻辑模块化**：新增 `filtering.py`，集中承载候选人筛选、评分、必要条件、薪资/经验/城市解析等纯规则逻辑
- **GUI 稳定性收口**：浏览器检测后台线程和运行收尾状态更新改为通过 Tk 主线程调度，降低偶发 UI 状态错乱风险
- **API 当前模型保存修复**：保存 API 配置时正确写入当前输入的模型名，避免当前模型仍保留旧值
- **年龄解析增强**：年龄筛选支持 `35岁`、`年龄：36 岁` 等无空格或带前缀格式
- **稳定回归扩充**：单元回归覆盖年龄边界、非统招本科、筛选兼容导出、候选人去重和打招呼状态合并

### v2.5 新增功能
- **浏览器连接重构**：动态调试端口 + 独立 Chrome profile，避免固定 `9222` 端口冲突并保留登录态
- **手动连接自动启动 Chrome**：点击检测/连接浏览器时自动启动 Chrome，并等待调试端口就绪
- **候选人年龄筛选**：需求解析、GUI 配置和筛选逻辑全链路支持最大年龄限制
- **学历与薪资规则收紧**：非统招学历识别更严格，薪资过滤调整为候选人期望最低薪资 >= 岗位上限 + 1K 即淘汰
- **发布流程加固**：发布前检查覆盖依赖、敏感文件、源码编译、稳定单元回归、导入烟测和工作区状态
