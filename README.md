...
- **AI 增强招聘需求解析**：粘贴招聘需求后，AI 会自动帮你补全和优化配置；未配置 AI 时使用本地规则解析
- **岗位优先项独立加分**："证券行业优先"、"全栈开发经验优先"等条件作为额外加分项，不再影响基础评分
- **必要条件智能解析**：硬性条件中的"金融投资行业经验，债券、基金、期货等"会自动识别为"满足任一即可"
- **模型连通性测试**：已保存的 AI 模型可右键批量测试，快速验证可用性
- **多接入方式支持**：同一服务商可同时配置多种接入方式

**体验优化**

- **新建岗位引导**：岗位配置页新增流程引导提示，按步骤操作更直观
- **解析摘要增加薪资**：解析成功后直接显示薪资范围
- **服务商默认模型更新**：DeepSeek、Kimi、智谱等更新为最新版本
- **运行控制页优化**：打招呼等级、AI 评估等显示更清晰

**问题修复**

- **切换服务商配置丢失**：修复切换时 API Key、模型名称显示为默认值的问题
- **切换模型崩溃**：修复切换已保存模型时程序崩溃的问题
...
- **模型下线检测**：自动检测已下线模型并提醒
- **批量连通性测试**：可一次测试多个模型，显示汇总结果
- **业务错误友好提示**：额度不足、未开通等错误会给出具体解决方案

**体验优化**

- **更新弹窗优化**：更新内容展示更清晰
- **测试连接提示优化**：错误信息更通俗易懂
- **批量测试结果精简**：只显示可用/不可用数量，不再逐条显示

**问题修复**

- **Windows 版本号显示**：修复版本号显示异常的问题
- **API Key 读取修复**：修复切换服务商后无法正确读取密钥的问题
