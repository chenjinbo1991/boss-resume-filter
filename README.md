...
> 当前发布版本：v2.11.4 三层扫描逻辑重构（版本号 v2.11.4）
...
- **三层扫描逻辑重构**：候选人提取从 API 优先翻转为 DOM 优先——第一层 listener + refresh 捕获首屏 API 数据，第二层 DOM 滚动建立候选人集合并按 geek_id 合并 listener 结构化字段，第三层 API 直调仅对仍缺结构化字段的候选人做后置补全；API 和 listener 数据不再引入 DOM 未出现的新候选人，确保自动打招呼和筛选只作用于页面可见的人
