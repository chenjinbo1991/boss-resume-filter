...
> 当前发布版本：v2.11.3 DOM 可点击扫描与页面跑偏检测（版本号 v2.11.3）
...
- **结构化补全说明**：图形界面运行控制页固定展示 `listener + refresh → DOM → API 兜底` 的结构化补全策略，不再暴露“安全扫描”开关；运行日志会记录 listener 捕获数量和接口地址
- **API 补全预算**：API 直调仅用于补全 DOM 已出现候选人的结构化字段，默认最多补全 5 页，补全页数上限为 10 页，页间保留随机间隔
...
- **结构化日志更聚焦**：运行日志只保留 listener/API 捕获数量、接口地址、命中 DOM 数量和分页兜底结果，清除 jobId、identity、API URL 探测等临时诊断输出
