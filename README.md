...
> 当前发布版本：v2.11.2 反风控增强（版本号 v2.11.2）
...
- **验证码智能识别**：打招呼时自动检测验证码弹窗，暂停操作并提示手动完成验证，完成后自动继续
- **API 熔断保护**：连续多次接口调用失败时自动暂停，避免触发平台风控，等待一段时间后自动恢复
- **打招呼限速**：控制打招呼节奏，模拟人工操作速度，降低被平台识别为自动化操作的风险
