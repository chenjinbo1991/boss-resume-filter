...
4. 点击“解析招聘需求”。
5. 检查系统自动提取的岗位名称、经验、学历、薪资、地点、技能关键词。
6. 明显不对的字段手工修正。
7. 保存配置。

解析功能是辅助，不是最终裁决。保存前必须人工检查，尤其是薪资上下限、必要条件、技能关键词权重。
...
如果只做规则筛选，可以跳过本节。  
如果要启用“AI 辅助评估”，必须先进入左侧“系统设置”配置模型。
...
数据统计适合看岗位规则质量。如果某个岗位优质率极低，通常不是市场上没人，而是规则过硬、关键词偏窄或 BOSS 当前推荐池不匹配。
