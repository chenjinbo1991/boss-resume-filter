...
启动成功后，窗口左下角会显示当前版本号，例如 `v2.10.2`。
...
3. 优先通过 API 直调分页获取候选人结构化数据；不可用时自动降级到接口监听；监听也不可用时回退到 DOM 滚动提取。
4. 对候选人执行硬条件过滤和评分。
5. 需要时调用大模型二次评估。
6. 按打招呼策略发送消息。
7. 保存候选人数据并导出 Excel。
...
v2.10.2 采用三级提取链路，优先使用结构化数据：

| 提取方式 | 触发条件 | 数据来源 |
| --- | --- | --- |
| API 直调 | 默认首选 | 从当前推荐页直接调用 BOSS 接口分页获取，数据最完整 |
| 接口监听 | API 直调失败时 | 刷新页面触发接口调用，捕获返回的结构化数据 |
| DOM 滚动 | 前两级均失败时 | 滚动页面提取可见卡片文本，依赖页面结构 |
...
在候选人行上双击，可以打开详情弹窗，查看完整的评分拆解（含 AI 调整分）、评分解释、命中证据、跟进状态和人工反馈。
...
| 操作 | 说明 |
| --- | --- |
| 双击候选人行 | 打开详情弹窗，查看评分拆解（含 AI 调整分）、评分解释、命中证据、跟进状态和人工反馈 |
...
Excel 导出包含完整候选人信息，v2.10.2 起列重排为 5 个区块：身份信息 → 候选人画像 → 评估结果 → 跟进状态 → 原始数据。新增列包括：风险提示、是否需人工确认、自动打招呼阻断原因，方便离线审阅和批量处理。
