...
- 通过 API 直调、接口监听、DOM 滚动三级链路提取候选人，优先使用结构化数据。
- 对符合阈值的候选人自动打招呼。
...
| 运行控制 | 连接浏览器、选择岗位、设置滚动轮次、读取上限、安全扫描和打招呼策略 |
...
v2.9.3 后，解析会主动过滤泛化词和噪声词，例如”人工智能””证券行业””数据清洗”等不会轻易被当成技术技能；学历、工作年限等基础门槛会归入基本信息，不再混进必要条件；”证券经验优先””大模型 Agent 经验优先”等优先项会作为额外加分项，不会稀释普通技能关键词匹配分。v2.10.1 进一步改进了 AI 评估的输入质量，候选人信息经过结构化压缩后传给大模型，评估质量和响应速度均有提升。
...
| 滚动轮次 | 安全扫描开启时默认 100、推荐 50-200；关闭安全扫描时默认 30、推荐 30-50 |
| 读取上限 | 仅关闭安全扫描走 API 直调时显示；默认 80，达到上限自动停止继续翻页 |
| 安全扫描 | 默认开启；跳过 API 直调，先用 listener + refresh 一次，失败后回退页面滚动 |
...
3. 安全扫描开启时，跳过 API 直调，先用 listener + refresh 一次捕获页面自然接口数据，失败后回退 DOM 滚动提取；安全扫描关闭时，优先通过 API 直调分页获取候选人结构化数据，默认单次最多读取 80 人，不可用时自动降级到接口监听，监听也不可用时回退到 DOM 滚动提取。
4. 对候选人执行硬条件过滤和评分。
5. 需要时调用大模型二次评估。
6. 按打招呼策略发送消息。
7. 保存候选人数据并导出 Excel。
...
当前提供两种提取方式：安全扫描默认跳过 API 直调，使用 listener + refresh 一次后按需回退 DOM 滚动，稳定性优先；关闭安全扫描后使用 API 优先三级链路，结构化数据更完整。

| 提取方式 | 触发条件 | 数据来源 |
| --- | --- | --- |
| listener 优先 | GUI 安全扫描默认开启 | 刷新页面一次触发自然接口调用，捕获返回数据；失败后回退 DOM 滚动 |
| DOM 滚动 | listener 或 API 均不可用 | 滚动页面提取可见卡片文本，依赖页面结构 |
| API 直调 | 关闭安全扫描或 CLI 默认模式 | 从当前推荐页直接调用 BOSS 接口分页获取，默认单次最多读取 80 人 |
| 接口监听 | API 直调首轮拿不到候选人时 | 刷新页面一次触发接口调用，捕获返回的结构化数据 |

API 直调不会触发页面刷新，不影响用户手动选择的招聘职位。刷新兜底方案会检查刷新前后的岗位标识，如果岗位变化会停止扫描并提示，避免静默抓取错误岗位的候选人。
...
v2.10.1 起，疑似非统招本科（如自考、成考、网络教育等）不再直接淘汰，改为标记风险提示。系统会在候选人详情和打招呼确认弹窗中展示风险提示，由招聘人员确认后再决定是否沟通。
...
单独打招呼会尝试在 BOSS 当前列表页自动滚动定位候选人卡片。定位失败时，先确认浏览器仍在对应岗位的推荐页面。候选人详情和打招呼确认弹窗会展示风险提示（如疑似非统招学历），避免误操作。
...
v2.10.1 起已移除 API 监听预热时的页面刷新，改为微滚动，不会再重置用户手动选择的招聘职位。如果仍出现职位重置，检查是否在运行过程中手动切换了 BOSS 页面的职位。
