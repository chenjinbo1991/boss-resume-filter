...
启动成功后，窗口左下角会显示当前版本号，例如 `v2.12.2`。
...
当前资格审查分为“明确合格、需人工确认、明确淘汰”三种状态。明确自考、成教、函授等非统招学历时直接淘汰；只有本科层级但缺少统招证据、或“专升本”但学习形式不明时转人工确认。系统会在候选人详情和打招呼确认弹窗中展示原因，需人工确认的候选人不会自动打招呼。
...
单独打招呼会优先使用筛选时保存的发送信息，失败时再尝试在 BOSS 当前列表页自动滚动定位候选人卡片。定位失败时，先确认浏览器仍在对应岗位的推荐页面。候选人详情和打招呼确认弹窗会展示需要人工确认的资格风险，避免误操作。
