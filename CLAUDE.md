...
- **Gitee Release 上传**：产物在哪个平台构建，就由该平台直接上传 GitHub + Gitee；本地发布不再下载中转 CI 对端产物，只轮询等待 Gitee 附件就绪。macOS ZIP/DMG 最多 2 路并发上传，小文件最多 3 路并发；上传超时 600s，4xx 不重试
- **Gitee 完整性校验**：发布主流程只校验附件齐全和 size 与 GitHub 一致，不回下载大文件；需要逐文件 SHA256 时手动运行 `python build.py --verify-gitee-integrity X.Y.Z`
- **Gitee Token**：本地使用环境变量 `GITEE_TOKEN`；GitHub Actions 使用同名 Repository Secret，CI 未配置时上传步骤失败
