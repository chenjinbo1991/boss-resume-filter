...
- **GitHub Actions 双平台自动打包**：推送 tag 后自动在 Windows + macOS runner 上并行打包，产物自动上传到同一个 Release（`.github/workflows/release.yml`）；CI 模式使用 `--ci --release` 跳过虚拟环境切换和 git 操作；通过 `paths`/`paths-ignore` 过滤，仅源码、依赖、嵌入文件（`.py`/`CHANGELOG.md`/JSON 配置）变更时触发，纯文档提交不触发构建
