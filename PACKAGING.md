...
**增量上传**：上传前比较远端附件的 size 和 created_at 与本地文件：大小一致且本地 mtime 不超过远端创建时间 5 分钟则跳过，否则删旧重传。避免重复上传和单文件失败后全量重传。

**覆盖发布**：重新发布同一版本时，增量比对后只重传有变化的文件；同时同步 Release 标题（以 CHANGELOG 为准，正文保留手动修改不覆盖）。

`latest.json` 字段说明：
- `downloads`：GitHub 下载链接（国际）
- `downloads_cn`：Gitee 下载链接（国内优先，`updater.py` 优先使用此字段）
- `assets`：产物元数据（`size` 和 `sha256`），`updater.py` 下载后校验完整性用
